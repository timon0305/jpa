...
class LCRReport extends Component {
  ...
  deleteLCR = (id, type) => {
    ...
    if (type === "lcr") {
      ...
      this.props.callApi(RestApi.deleteLataNpanxxReport2ById, response => {
        if (response.ok){
  ...
  columns = [
    ...
    },
    {
      ...
      Cell: props => <div className="text-center">
        <Button size="sm" color="primary" onClick={() => this.viewLCR(props.value, "lcr")}>View</Button>
        <Button size="sm" color="danger" className="ml-2" onClick={() => this.deleteLCR(props.value, "lcr")}>Delete</Button>
  ...
  columns2 = [
    ...
    },
    {
      ...
      accessor: 'id',
      Cell: props => <div className="text-center">
        <Button size="sm" color="primary" onClick={() => this.viewLCR(props.value, "lata")}>View</Button>
        <Button size="sm" color="danger" className="ml-2" onClick={() => this.deleteLCR(props.value, "lata")}>Delete</Button>
        <Button size="sm" color="success" className="ml-2" onClick={() => {
          this.downloadForm.action = process.env.REACT_APP_API_ENDPOINT + "cprgen/lata_npanxx_report_2/" + props.value + "/download";
          this.textInput.value = this.props.auth.token;
          this.downloadForm.submit();
          this.textInput.value = "";
        }}>Download</Button>
  ...
  _viewData2 = () => {
    let sorts = [];
    let filters = [];
    if (this.state.modal2.sort.length) {
      let sorted = this.state.modal2.sort;
      let column = sorted[0][0].id;
      let direction = sorted[0][0].desc;
      if (direction === false) direction = "asc";
      else if (direction === true) direction = "desc";
    ...
    if (this.state.modal2.filter.length) {
      let filtered = this.state.modal2.filter;
      filtered = filtered[0];
      let filter = {};
      for (let i = 0; i < filtered.length; i++) {
        if (filtered[i].id === "role") {
    ...
    let data = {
      page: this.state.modal2.page,
      pageSize: this.state.modal2.pageSize === 0 ? 10 : this.state.modal2.pageSize,
      sorts: sorts,
      filters: filters
    };
  ...
  handleLataRefresh = () => this.fetchLataNpanxx();
  ...
  fetchLataNpanxx = () => {
    ...
    this.fetchTime = setTimeout(this._fetchLataNpanxx, 600)
  };
  ...
  render() {
    return (
      <Row>
        <Col lg="12">
          <Card>
            ...
            <CardFooter>
              ...
              <Button size="md" color="primary" onClick={this.lataNpanxxReport1} className="ml-3">LATA/NPANXX Report 1</Button>
              <Button size="md" color="primary" onClick={this.lataNpanxxReport2} className="ml-3">LATA/NPANXX Report 2</Button>
            </CardFooter>
          </Card>
          <Card>
            <CardHeader>
          ...
          <Card>
            <CardHeader>
              <Row>
                <Col><strong className="card-title-big">View LATA/NPANXX Report 1</strong></Col>
                <Col>
                  <div className="text-right">
            ...
            <CardBody>
              <ReactTable
                ...
                columns={this.lataNpanxxcolumns}
                defaultPageSize={10}
                onFilteredChange={(filter) => {
                  console.log(filter);
                  let filters = [];
                ...
                onFetchData={this.fetchLataNpanxx}
              />
            </CardBody>
          </Card>
          <Card>
            <CardHeader>
              <Row>
                <Col><strong className="card-title-big">LATA/NPANXX Report 2 List</strong></Col>
                <Col>
                  <div className="text-right">
        ...
        <ViewLCRModal
          isOpen={this.state.isLcr?this.state.modal.isOpen: this.state.modal2.isOpen}
          toggle={this.state.isLcr ? this.toggleModal : this.toggleModal2}
          columns={this.state.isLcr ? this.LCRcolumns : this.lataNpanxxColumns}
          data={this.state.isLcr ? this.state.modal.data : this.state.modal2.data}
          fetchData={this.state.isLcr ? this.viewData : this.viewData2}
          total_page={this.state.isLcr ? this.state.modal.total_page : this.state.modal2.total_page}
          handler={this.state.isLcr?this.handleUpdate: this.handleLataUpdate}
        />
        <RenameRateModal
